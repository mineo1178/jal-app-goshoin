...
const JapanMap = memo(({ progressData, onSelectAirport, regionStats }) => {
  // SVG ViewBox設定
  const WIDTH = 600;
  const HEIGHT = 500;
  ...
  const project = (lat, lon) => {
    ...
    // 緯度30度付近の種子島・屋久島も含めて枠内に入れるため閾値を32に設定
    if (lat < 32) {
       // 離島エリア用プロジェクション（枠内に収めるためのローカル座標系）
       // 緯度: 24.0(南端:波照間付近) ~ 31.5(北端:種子島以北) -> y: 230 ~ 50
       // 経度: 122.5(西端:与那国付近) ~ 131.5(東端:大東諸島付近) -> x: 40 ~ 200
...
const AirportDetailModal = ({ selectedAirport, progressData, onClose, toggleCollection, updateDate, updateMemo }) => {
...
const Dashboard = ({ stats, progressData, onSelectAirport }) => (
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-end mb-6">
          <div className="mb-1">
            ...
            <p className="text-gray-400 text-sm mt-1">あと <span className="font-bold text-gray-600">{stats.total - stats.collected}</span> 空港で制覇！</p>
          </div>
          <div className="text-right flex items-baseline">
            <span className="text-6xl font-black text-red-600 tracking-tighter leading-none">{stats.percentage}</span>
      ...
      <div className="grid grid-cols-2 gap-3">
        {Object.entries(stats.byRegion).map(([region, data]) => {
          ...
          const styles = REGION_COLORS[region] || 'bg-gray-500';
    ...
    <div className="pb-24 space-y-4 px-1">
      ...
      <div className="space-y-3">
        {filteredAirports.map((airport) => {
          ...
          const styles = REGION_COLORS[airport.region] || 'bg-gray-500';
